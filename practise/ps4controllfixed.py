...
# gornata promenliwa moje da ni tryabwa za resset
autopilot=False
...
while True:
    ...
    if a.get_button(0)==True:
        ...
        pygame.display.quit()
        break
    # buton for info about game?
    # if a.get_button(2)==True:
    #     if len(button_info.get_list())!=0:
    #         if (button_info.is_diferent_date()):
    #             spawning_actors.deleteAllelements(button_info.get_list())
    #     if len(button_info.get_list())==0:       
    #         button_info.add_new_date_now()
    #         if button_info.get_button() is True:
    #             button_info.set_state_of_pushed_button(False)
    #         else:
    #             button_info.set_state_of_pushed_button(True)
    #         if button_info.get_button() is True:
    #             print("Natisnal si info")
    #             print_info()
    #         elif button_info.get_button() is False:
    #             print("Zatwarya6 go infoto")
    # useless e towa otgore :) prosto sus startiraneto na skripta 6e se printnat kontrolite :)
